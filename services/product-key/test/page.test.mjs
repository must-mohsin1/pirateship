...
test("the CSP-compatible page loads behavior from an external script", () => {
  assert.match(html, /<script src="page\.js"><\/script>/);
  assert.doesNotMatch(html, /<script>(?:.|\n)*?<\/script>/);
  assert.match(source, /document\.querySelector\("\.rail"\)/);
});
